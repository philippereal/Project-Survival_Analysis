...
En terme d'accruancy (erreur de prévision par rapport aux données de test) les modèles sont relativement similaires.
Le fait est que l'on n'est pas parvenu à obtenir des résultats stables au niveau des critères AUC et d'accruancy. Probablement dû à la manière dont les jeux de données Train et Test ont été construits. Il est très possible que ce soit la conséquence d'une mauvaise stratification. Et donc le choix de modèle est rendu difficile par l'instabilité des résultats.
Le seul résultat stable se situe au niveau de la prédiction de survie qui est nettement supérieure dans le cas des modèles de Cox d'environ (10% à 15%) par rapport au modèle logit. 
