...
Traitements des factor.region et race sont des variables categorielles.
```{r}
...
On cherche à représenter la courbe de l'estimateur de Kaplan-Meyer pour la fonction de survie : âge de contraction de la pneumonie (the age at pneumonia).  

Pour définir l'estimateur de Kaplan-Meyer on se donne :
* Une durée T avec la fonction de survie associée $\bar{F}$
* Un temps de censure C, independant de T
pour la fonction de survie
...
On peut tracer la courbe de Kapplan-Meyer
...
On obtient à partir de l'estimateur KM de la pobabilité de survie,
la probabilité qu'un nouveau né n'ai pas développé une pneumonie à 6 mois peut être lu dans le résultat de la sortie suivante:

```{r}
summary(kmsurvival)
```
La probabilité estimée de n'avoir pas eu de pneumonie à 6 mois est de 0.981 (CI = [0.976 , 0.986])

### Question 3

La variable dummy Z vaut 1 si les enfants sont allaités à la naissance et 0 sinon (ils ne le sont jamais).
Pour cela on considère la variable : wmonth qui indique le mois à a partir duquel l'enfant est sevré.
On ajoute notre nouvelle variable Z au jeu de donnée initiale pneumon.
Auparavant Z est transformée en factor par le biais de la fonction recode (avec 1 -> Allaitement /  0 ->Pas d'allaitement)

```{r}
pneumon = mutate(pneumon , Z = recode(factor(wmonth > 0 ), "TRUE"= "Allaitement", "FALSE" ="Pas d'allaitement"))
```

Comme à la question précédente on utilse la fonction survfit, mais cette fois ci sur nos deux groupes de population, obtenus par différentiation à partir de la variable Z.

```{r}
KM_fit =survfit(Surv(chldage,hospital)~Z, data = pneumon)
summary(KM_fit)
```
...
Ce qui confirme la première inpression donnée par le graphique précédent.

### Question 4
Pour tester l'hypothèse $H_0 : \beta_{breastf}^* =0$ on va maitenant utiliser le maximum de vraissemblance et 

On test à nouveaux  mais cette  avec un modèle de Cox.
```{r}
...
### Question 5
Modèles de Cox à 2 facteurs, en prenant pour variable explicative la variable Z et successivement, une à une chacune des autres variables explictives: mthage, urban, alcohol, smoke, region, poverty, bweight, race, education, nsibs, wmonth, sfmonth
...
#variable explicatives: Z + mthage
...
#### variable explicatives: Z + bweight
#variable explicatives: Z + bweight
...
#### variable explicatives: Z + wmonth
#variable explicatives: Z + wmonth
